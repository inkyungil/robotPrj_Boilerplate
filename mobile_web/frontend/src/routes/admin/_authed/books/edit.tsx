...
export const Route = createFileRoute("/admin/_authed/books/edit")({
  validateSearch: (search: Record<string, unknown>) => {
    return {
      id: typeof search.id === "string" ? search.id : "",
    };
  },
  head: () => ({ meta: [{ title: "Libi Admin — 도서 수정" }] }),
...
function EditBookPage() {
  ...
  useEffect(() => {
    ...
    async function loadBook() {
      try {
        const b = await adminApi.getBook(Number(id));
        setTitleKr(b.title.KR);
        setTitleEn(b.title.EN || "");
        setTitleZh(b.title.ZH || "");
        setTitleVi(b.title.VI || "");
        setAuthor(b.author);
        ...
        setSummaryKr(b.summary?.KR || "");
        setSummaryEn(b.summary?.EN || "");
        setSummaryZh(b.summary?.ZH || "");
        setSummaryVi(b.summary?.VI || "");
        setForWhomKr(b.forWhom?.KR?.join(", ") || "");
        setForWhomEn(b.forWhom?.EN?.join(", ") || "");
        setForWhomZh(b.forWhom?.ZH?.join(", ") || "");
        setForWhomVi(b.forWhom?.VI?.join(", ") || "");
        setFetching(false);
      } catch (err: any) {
        toast.error(err.message || "도서 정보를 불러오지 못했습니다.");
        void navigate({ to: "/admin/books" });
      }
    }
  ...
  const updateMutation = useMutation({
    mutationFn: (data: CreateBookInput) => adminApi.updateBook(Number(id), data),
    onSuccess: () => {
    ...
    onError: (err: any) => {
      toast.error(err.message || "도서 정보 수정에 실패했습니다.");
      setLoading(false);
    },
  });
  ...
  if (fetching) {
    return (
      <AdminShell title="도서 수정">
        <div className="flex min-h-[400px] items-center justify-center">
          <div className="flex flex-col items-center gap-2 text-slate-400">
            <Loader2 className="size-8 animate-spin text-slate-500" />
            <span className="text-sm">도서 정보를 불러오는 중...</span>
          </div>
        </div>
      </AdminShell>
    );
    ...
    <AdminShell title="도서 수정">
      <div className="mx-auto max-w-4xl space-y-6">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-bold text-slate-800">도서 수정</h1>
        </div>
